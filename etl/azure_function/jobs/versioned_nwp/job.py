...
class VersionedNwpETL:
    ...
    def get_available_forecasts(self) -> pd.DataFrame:
        ...
        forecasts = forecasts[forecasts["model_run_time_utc"] >= (pd.Timestamp.now() - pd.Timedelta("PT6H"))]

        forecasts = forecasts[forecasts["model_run_time_utc"] == forecasts["model_run_time_utc"].min()]
    ...
    def get_new_forecasts(self) -> pd.DataFrame:
        ...
        new_forecasts = available_forecasts[~available_forecasts["run_id"].isin(downloaded_forecasts["run_id"])]
    ...
    def run(self):
        ...
        for _, forecast in new_forecasts.iterrows():
            ...
            with NamedTemporaryFile(suffix=".grib") as tmp_download_file:
                ...
                # log run
                self.versioned_nwp_log.write(
                    df=forecast.to_frame().T
                )
