    ...
    def run(self):
        ...
        for forecast in new_forecasts.iter_rows(named=True):
            ...
            with NamedTemporaryFile(suffix=".grib") as tmp_download_file:
                ...
                model_run_time_utc = forecast["model_run_time_utc"].strftime("%Y%m%dT%H%M%SZ")
                time_utc = forecast["time_utc"].strftime("%Y%m%dT%H%M%SZ")


                forecast["output_path"] = f"uch/nwp/model_run_time_utc={model_run_time_utc}/time_utc={time_utc}/{model_run_time_utc}_{time_utc}.nc"
