from azure.storage.filedatalake import DataLakeServiceClient, DataLakeFileClient, DataLakeDirectoryClient
...
class Field:
    ...
    def is_between(self, left: object, right: object) -> Rule:
        return Rule(
            func=lambda partition: left <= partition[self.name] <= right
        )

    def __eq__(self, other) -> Rule:
        return Rule(
            func=lambda partition: partition[self.name] == other
        )
    ...
    def __contains__(self, other) -> Rule:
        return Rule(
            func=lambda partition: other in partition[self.name]
        )
...
class Dataset:
    ...
    def _partition_to_file_path(self, partition: dict) -> str:
        ...
        partition_path = "/".join([f"{key}={value}" for key, value in casted_partition.items()])
    ...
    def write(self, dataset: xr.Dataset):
        ...
        for partition_values, stacked_partition_dataset in stacked_dataset.groupby("partition"):
    ...
    def _list_paths(self) -> list[str]:
        files = self.datalake._fs_client.get_paths(path=f"{self.name}/_data", recursive=True)
        return [file.name for file in files if not file.is_directory]
    
    def _yield_datasets(self, pushdown: Rule = ALL):
        valid_paths = [path for path in self._list_paths() if pushdown.contains(self._file_path_to_partition(path))]
    ...
    def read(self, pushdown: Rule = ALL):
        ...
        return xr.combine_nested(datasets, combine_attrs="drop_conflicts", concat_dim="time_utc")
...
class DataLake:
    ...
    def __init__(self, storage_account_name: str, container_name: str):
        ...
        self._fs_client = self._dls_client.get_file_system_client(file_system=container_name)
    ...
    def create_dataset(self, name: str, partition_schema: dict = None, overwrite: bool = False, promp_user: bool = True) -> Dataset:
