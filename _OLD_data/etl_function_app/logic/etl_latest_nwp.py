...
class ETL:
    ...
    def get_new_batches(self) -> pl.DataFrame:
        ...
        new_batches = available_batches.join(
            transformed_batches, on="model_run_time_utc", how="anti"
        )

        logger.info(
            f"{len(new_batches)} new forecasts. {len(available_batches)-len(new_batches)} existing forecasts."
        )
    ...
    def run(self):
        ...
        new_batches = self.get_new_batches().sort(by="model_run_time_utc")
        ...
        for batch in new_batches.iter_rows(named=True):
            ...
            pushdown = dl.field("model_run_time_utc") == np.datetime64(
                model_run_time_utc
            )
